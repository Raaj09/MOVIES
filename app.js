...
const movie Details = request.body;
...
movieName: dbObject.movie_name,
Lead Actor: dbObject.lead_actor,
  };
};
...
console.log(movieId);
response.send(convertDbObject To ResponseObject (movie));
});
...
//5 Deletes movie from the movie table based on the movie ID
...
WHERE
movie_id = ${movieId};`;
await db.run(delete MovieQuery);
response.send("Movie Removed");
});

const convertDirector Details PascalCase = (dbObject) ⇒ {
return {
directorId: dbObject.director_id,
...
const movies = await db.all(get DirectorMovieQuery);
